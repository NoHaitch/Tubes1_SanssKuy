...
class BotGreedyPath(BaseLogic):
    ...
    def getClosestDiamond(self, board_bot: GameObject, board: Board) -> GameObject:
        diamonds = self.getDiamonds(board)
        if not diamonds:
            return None
        return min(diamonds, key=lambda diamond: self.distance(board_bot, diamond, board))
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        temp:GameObject = self.getClosestDiamond(board_bot, board)
        diamond_distance = self.distance(board_bot, temp, board)
        if(diamond_distance > self.distance(board_bot, self.getDiamondButton(board), board) + 3):
        ...
        if(self.getEmptyInventorySpace(board_bot) >= 2):
            if temp != None:
                print(f"Going to Diamond. Location : {temp.position}")
                return self.moveToObjective(board_bot, temp, board) 

        if(self.getEmptyInventorySpace(board_bot) >= 1 and temp.properties.points == 1 and diamond_distance <= 2):
            print(f"Going to Diamond. Location : {temp.position}")
            return self.moveToObjective(board_bot, temp, board) 
